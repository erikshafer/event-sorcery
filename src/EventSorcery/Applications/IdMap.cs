...
internal class IdMap<TId> : Dictionary<Type, Func<object, CancellationToken, ValueTask<TId>>>
    where TId : AggregateId
{
    public void AddCommand<TCommand>(GetIdFromCommand<TId, TCommand> getId)
    ...
    public void AddCommand<TCommand>(GetIdFromCommandAsync<TId, TCommand> getId)
        where TCommand : class
    {
        TryAdd(
            typeof(TCommand),
